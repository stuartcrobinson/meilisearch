...
These collected metrics are persisted (e.g., in Prometheus) for historical analysis, dashboarding in Grafana, and alerting. Crucially, they also serve as primary inputs for the Automated Orchestrator Service. The Orchestrator consumes these metrics—either directly from the monitoring system for real-time data or via recent aggregates/peaks stored in GMD fields like current_load_metrics_json, current_engine_load_metrics_json, and current_index_size_gb for quick decision-making—to make intelligent, automated decisions about resource allocation, index placement, and triggering migrations (SIS) to maintain performance SLAs and optimize resource utilization.
      ...
      *   **Alerting:**
          *   **Centralized Alerting:** The primary monitoring system (e.g., Prometheus/Alertmanager) triggers alerts based on predefined thresholds for collected metrics (e.g., `SearchNodeVM` or `FRRNodeVM` down, search engine unresponsive, sustained high error rates, full disk, GMD issues, SIS failures). These alerts can integrate with PagerDuty, Slack, etc.
          *   **Node-Initiated Fast Alerts (for Critical Spikes):** In addition to centralized polling, agents on `SearchNodeVMs` (or the `DeployedEngines` themselves) can be configured with highly sensitive local thresholds for critical resource spikes (e.g., extreme CPU/memory usage persisting for a very short duration). If breached, these agents can send an immediate, high-priority alert directly to the alerting system or a dedicated Orchestrator endpoint. This allows for faster reaction to sudden critical overload conditions that might occur between standard metric collection intervals, enabling the Orchestrator to take more immediate preemptive actions like temporarily deprioritizing the node for new workloads.
