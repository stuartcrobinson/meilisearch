...
use deserr::{DeserializeError, Deserr, ErrorKind, MergeWithError, ValuePointerRef}; // Remove DeserializeProxy import
...
impl<'de> Deserialize<'de> for WildcardSetting {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let setting: Setting<Vec<String>> = Setting::deserialize(deserializer)?;
        match setting {
            Setting::Set(ref vec) if vec.len() == 1 && vec[0] == "*" => Ok(Self(Setting::Reset)),
            other => Ok(Self(other)),
        }
    }
}
...
// Manual Deserr implementation for WildcardSetting
impl<E: DeserializeError> Deserr<E> for WildcardSetting {
    fn deserialize_from_value<V: deserr::IntoValue>(
        value: deserr::Value<V>,
        location: ValuePointerRef<'_>,
    ) -> Result<Self, E> {
        // Use the Deserr implementation of the inner type
        let setting: Setting<Vec<String>> = Setting::deserialize_from_value(value, location)?;

        // Apply custom logic: ["*"] means Reset
        match setting {
            Setting::Set(ref vec) if vec.len() == 1 && vec[0] == "*" => Ok(Self(Setting::Reset)),
            other => Ok(Self(other)),
        }
