...
fn query_batches_from_and_limit() {
    ...
    let kind = index_creation_task("doggo", Some("bone")); // Wrap in Some()
    let _task = index_scheduler.register(kind, None, false).unwrap();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "registered_the_first_task");
    let kind = index_creation_task("whalo", Some("plankton")); // Wrap in Some()
    let _task = index_scheduler.register(kind, None, false).unwrap();
    snapshot!(snapshot_index_scheduler(&index_scheduler), name: "registered_the_second_task");
    let kind = index_creation_task("catto", Some("his_own_vomit")); // Wrap in Some()
    let _task = index_scheduler.register(kind, None, false).unwrap();
