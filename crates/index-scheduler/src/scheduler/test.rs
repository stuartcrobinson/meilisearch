...
mod msfj_sis_scheduler_e2e_tests {
    ...
    async fn test_e2e_snapshot_create_import_verify() {
        ...
        file.persist().unwrap();
        // Use string literal directly for 'static lifetime
        let task = replace_document_import_task("source_e2e", Some("id"), 0, documents_count);
        let _task_id = index_scheduler.register(task, None, false).unwrap().uid;
        handle.advance_one_successful_batch();
        ...
        // === 5. Verify Indexes ===
        // Get indexes using literals (string literal for source, borrow String for target)
        let source_index = index_scheduler.index("source_e2e").unwrap();
